...
if test -z "${pfm}" -a -z "${title}" ; then
  ...
  program_id=`xmllint http://radiko.jp/v3/program/station/weekly/${station_id}.xml --xpath "//${node}[contains(text(), '${value}') and not(.=preceding::${node})]/parent::node()/@id" | sed -e "s/ id=//"`
